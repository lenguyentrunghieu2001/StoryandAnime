...
if ($countprev = mysqli_num_rows($sql_prev_chapter) > 0) {
    ...
    if ($row_prev_chapter['money'] == 0 || $count = mysqli_num_rows($sql_chapter_taikhoan) > 0) { ?>
        <div class="option next-chapter" onclick="window.location.href='./Chapter/handleHistoryChap.php?idchap=<?= $row_prev_chapter['idchap'] ?>&taikhoanuser=<?= $taikhoanuser ?>&idtruyen=<?= $idtruyen ?>'"><i class="fa-solid fa-arrow-left"></i> Chương trước</div>
        <?php
    } else {
        if ($row_taikhoan['money'] >= $row_prev_chapter['money']) {
        ?>
            <div class="option next-chapter" onclick="if (confirm('Chap có giá <?= $row_prev_chapter['money'] ?> và tài khoản hiện tại bạn đang còn <?= $row_taikhoan['money'] ?>đ')) {return window.location.href='DetailStory/lockKeyChapter.php?idchap=<?= $row_prev_chapter['idchap'] ?>&taikhoanuser=<?= $taikhoanuser ?>&id=<?= $idtruyen ?>' }"><i class="fa-solid fa-arrow-left"></i> Chương trước</div>
        <?php
        } else { ?>

            <div class="option next-chapter" onclick="alert('tài khoản của bạn không đủ vui lòng nạp thêm')"><i class="fa-solid fa-arrow-left"></i> Chương trước</div>
    <?php
        }
    }
    ?>
...
?>
    <div class="option next-chapter" style="opacity: 0.5;"><i class="fa-solid fa-arrow-left"></i> Hết chương</div>
