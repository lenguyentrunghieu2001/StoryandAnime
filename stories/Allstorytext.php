...
<div class="content text-center">
    <div class="container">
        <h1 class="title">TRUYỆN CHỮ</h1>
        <div class="story">
            <?php
            $sql_story_count = mysqli_query($con, "SELECT * FROM tbl_story WHERE type = 0 ORDER BY id");
            ...
            $page = $_GET['page'];
            $startpage = ($page - 1) * 12;
            $sql_story = mysqli_query($con, "SELECT * FROM tbl_story WHERE type = 0 ORDER BY id DESC LIMIT $startpage,12");
            $i = 0;
        ...
        <div style="display: flex; justify-content: center; align-items: center;">
            ...
            ?>
                <button class="btn" style="background: #3785c9;" onclick="window.location.href='Allstorytext.php?page=<?= $pageprev ?>'">&laquo;</button>
            <?php
            }
            ?>
            <!-- for -->
            ...
            for ($k = 1; $k <= $avgcount; $k++) {
                if ($k == $_GET['page']) {            ?>
                    <button style="margin: 20px 3px; border: 1px solid;background: #3785c9;" class="btn" onclick="window.location.href='Allstorytext.php?page=<?= $k ?>'"><?= $k ?></button>
                <?php } else {
                ?>
                    <button style="margin: 20px 3px; border: 1px solid" class="btn" onclick="window.location.href='Allstorytext.php?page=<?= $k ?>'"><?= $k ?></button>
            <?php
                }
            } ?>
            <!-- next -->
            ...
            ?>
                <button class="btn" style="background: #3785c9;" onclick="window.location.href='Allstorytext.php?page=<?= $pageprev ?>'">&raquo;</button>
            <?php
            }
            ?>
        </div>
